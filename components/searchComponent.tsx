            ...
            <div className="max-w-sm mx-auto space-y-6">
                {branches.length > 0 && (
                    <SelectBox
                        ...
                        defaultValue={branch}
                    >
                        {branches.map((item: any) => (<SelectBoxItem key={item.id} value={item.id.toString()} text={item.name} />))}
                    </SelectBox>
                )}
            </div>
            <div className="flex-1 flex flex-row-reverse gap-2">
                <button className="btn btn-primary min-h-[2.5rem] h-[2.5rem]" onClick={handleSearch}>Search</button>
                <button className="btn btn-error min-h-[2.5rem] h-[2.5rem]" onClick={handleClear}>Clear</button>
            </div>
        </div>
    )
}
